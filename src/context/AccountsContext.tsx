...
import {AccountsState, Account, Transaction, FilterOptions, TransferRequest} from "../types/banking.types";
...
interface AccountsContextValue {
  ...
  transferMoney: (transferData: TransferRequest) => Promise<{success: boolean; message: string}>;
}
...
export const AccountsProvider: React.FC<{children: ReactNode}> = ({children}) => {
  ...
  const loadTransactions = useCallback(async (accountId: string): Promise<void> => {
    try {
      dispatch({type: "SET_LOADING", payload: true});
  ...
  const applyFilters = useCallback(
    (filters: FilterOptions): void => {
      ...
      if (filters.transactionType !== "all") {
        filteredTransactions = filteredTransactions.filter((transaction) => transaction.type === filters.transactionType);
      }
  ...
  const transferMoney = useCallback(
    async (transferData: TransferRequest): Promise<{success: boolean; message: string}> => {
      try {
        dispatch({type: "SET_LOADING", payload: true});
